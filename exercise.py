...
class SpeedCalculator(QWidget):
    def __init__(self):
        ...
        self.unit_combo.addItems(["Metric(km)", "Imperial(mile)"])
    ...
    def calculate(self):
        #Get distance and time from the imput box
        distance = float(self.distance_input.text())
        time = float(self.time_input.text())

        #Calculate averaage speed
        speed = distance / time
        ...
        if self.unit_combo.currentText() == 'Metric (km)':
            speed = round(speed, 2)
            unit = 'km/h'
        elif self.unit_combo.currentText() == 'Imperial (mile)':
